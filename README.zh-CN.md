...
浏览器打开 `http://localhost:18700`（Docker）或 `http://localhost:18788`（本机安装），首次启动会自动生成管理员账户，凭据将显示在容器日志中。
...
| 端口 | 服务 | 说明 |
| :--- | :--- | :--- |
| `18700` → `18788` | ClawDeckX Web UI | 主界面（宿主机 18700 → 容器内 18788） |
...
| `OCD_PORT` | `18788` | ClawDeckX 监听端口（容器内部） |
